...
const managerCanManageEmployee = async (managerId: number, employeeId: number): Promise<boolean> => {
  const managementRepo = AppDataSource.getRepository(UserManagement);

  const link = await managementRepo.findOne({
    where: {
...
export const cancelLeave = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    // ===== MANAGER OWNERSHIP CHECK (STRICT) =====
if (role === 'manager') {
  const managerId = req.user!.userId;
  const employeeId = leave.user.userId;

  const managementRepo = AppDataSource.getRepository(UserManagement);

  const managesCount = await managementRepo
    .createQueryBuilder('um')
    .innerJoin('um.manager', 'm')
    .innerJoin('um.user', 'u')
    .where('m.userId = :managerId', { managerId })
    .andWhere('u.userId = :employeeId', { employeeId })
    .getCount();

  if (managesCount === 0) {
    logClientError(
      403,
      `Manager ${managerId} attempted to approve leave for non-managed employee ${employeeId}`,
      req.originalUrl,
      req.method,
      managerId
    );
    return res.status(403).json({
      error: 'You are not authorised to approve leave for this employee',
    });
  }
}
// ===== END MANAGER OWNERSHIP CHECK =====
