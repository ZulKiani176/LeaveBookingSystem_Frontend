...
export const updateAnnualLeaveBalance = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const userId = parseInt(req.params.userId, 10);
    if (!userId) {
      logClientError(400, "Invalid userId", req.originalUrl, req.method, req.user?.userId);
      return res.status(400).json({ error: "Invalid userId" });
    }

    // ✅ accept the field your frontend sends
    const raw = req.body?.annualLeaveBalance;

    // Convert properly (handles "20" as well as 20)
    ...
    return res.json({
      message: `Annual leave balance updated`,
      data: { userId, annualLeaveBalance: user.annualLeaveBalance },
    });
  } catch (err) {
    console.error(err);
    return res.status(500).json({ error: "Internal server error" });
